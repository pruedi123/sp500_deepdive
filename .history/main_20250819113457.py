...
orig_head = df_raw[date_col_name].head(10).tolist()
orig_tail = df_raw[date_col_name].tail(10).tolist()

# --- Standardize the Date column to one format (YYYY-MM-DD) ---
# Work on a copy-like Series
_s = df_raw[date_col_name].astype(str).str.strip()
_d = pd.to_datetime(_s, errors="coerce")

# Second pass: explicit US short format like 10/1/24
_mask_nat = _d.isna()
if _mask_nat.any():
    _d2 = pd.to_datetime(_s[_mask_nat], format="%m/%d/%y", errors="coerce")
    _d.loc[_mask_nat] = _d2

# Third pass: four-digit year US format like 10/01/2024
_mask_nat = _d.isna()
if _mask_nat.any():
    _d3 = pd.to_datetime(_s[_mask_nat], format="%m/%d/%Y", errors="coerce")
    _d.loc[_mask_nat] = _d3

# Fourth pass: year-month strings like 2024-10
_mask_nat = _d.isna()
if _mask_nat.any():
    _d4 = pd.to_datetime(_s[_mask_nat], format="%Y-%m", errors="coerce")
    _d.loc[_mask_nat] = _d4

# Two-digit year fix (avoid 2068-style future years)
from datetime import datetime
_current_year = datetime.now().year
_two_digit_mask = _s.str.match(r"\d{1,2}/\d{1,2}/\d{2}$", na=False)
_future_mask = _two_digit_mask & _d.notna() & (_d.dt.year > _current_year + 1)
if _future_mask.any():
    _d.loc[_future_mask] = _d.loc[_future_mask] - pd.offsets.DateOffset(years=100)

# Excel serials fallback
_mask_nat = _d.isna()
if _mask_nat.any():
    _as_num = pd.to_numeric(_s[_mask_nat], errors="coerce")
    if _as_num.notna().any():
        _d5 = pd.to_datetime(_as_num, origin="1899-12-30", unit="D", errors="coerce")
        _d.loc[_mask_nat] = _d5

# If the majority are first-of-month, normalize to first-of-month for consistency
_tmp = _d.dropna()
if not _tmp.empty:
    _day_is_one_ratio = (_tmp.dt.day == 1).mean()
    if _day_is_one_ratio >= 0.90:
        _d = _d.dt.to_period("M").dt.to_timestamp()

# Final standardized string format
_df_std = df_raw.copy()
_df_std[date_col_name] = _d.dt.strftime("%Y-%m-%d")

# Update df_raw in-place to use standardized dates going forward
# (downstream code will parse these strings consistently)
df_raw[date_col_name] = _df_std[date_col_name]

st.success("Standardized 'Date' to a single format: YYYY-MM-DD.")

# Offer a download of the standardized CSV
_stand_bytes = _df_std.to_csv(index=False).encode("utf-8")
st.download_button(
    "Download standardized CSV",
    _stand_bytes,
    file_name="market_data_clean_std.csv",
)

# Now show original vs standardized samples
st.write("Using date column:", f"`{date_col_name}`")
st.write("Original 'Date' head:", orig_head)
st.write("Original 'Date' tail:", orig_tail)
st.write("Standardized 'Date' head (YYYY-MM-DD):", df_raw[date_col_name].head(10).tolist())
st.write("Standardized 'Date' tail (YYYY-MM-DD):", df_raw[date_col_name].tail(10).tolist())
