...
# --- Compute averages for decline and duration columns BEFORE formatting (robust parsing) ---
average_decline = None
average_duration = None

# Detect relevant columns by substring (handles typos like "unempolyment")
...
# Averages using numeric coercion (work even if source is string)
if decline_cols:
    _decline_numeric = pd.to_numeric(bear_filtered[decline_cols[0]], errors="coerce")
    average_decline = _decline_numeric.mean()
if duration_cols:
    _duration_numeric = pd.to_numeric(bear_filtered[duration_cols[0]], errors="coerce")
    average_duration = _duration_numeric.mean()

# --- Format Decline/Unemployment columns as percentages with one decimal place (robust) ---
percent_cols = [col for col in bear_filtered.columns if ("decline" in col.lower() or "unemp" in col.lower())]
for col in percent_cols:
    # Normalize to string, remove existing % and commas, trim
    s = bear_filtered[col].astype(str).str.replace('%', '', regex=False).str.replace(',', '', regex=False).str.strip()
    nums = pd.to_numeric(s, errors="coerce")
    # If values look like fractions (e.g., -0.192), scale to percent
    # Heuristic: if mean abs <= 2, treat as fraction and multiply by 100
    mean_abs = nums.abs().mean(skipna=True)
    if pd.notnull(mean_abs) and mean_abs <= 2:
        nums = nums * 100
    # Format as percentage with one decimal place
    bear_filtered[col] = nums.map(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")
# ---
...
# Display averages for decline and duration columns (using pre-format numeric values)
