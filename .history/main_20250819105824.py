...
default_end = "2025-07"
# Compute indices for selectboxes
if default_begin in available_months:
    begin_index = available_months.index(default_begin)
else:
    begin_index = 0
if default_end in available_months:
    end_index = available_months.index(default_end)
else:
    end_index = len(available_months) - 1
begin = st.sidebar.selectbox("Start (YYYY-MM)", options=available_months, index=begin_index)
end = st.sidebar.selectbox("End (YYYY-MM)", options=available_months, index=end_index)
