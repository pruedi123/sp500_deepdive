...
# --- Format columns containing 'decline' or 'unemployment' as percentages if appropriate ---
for col in bear_filtered.columns:
    col_lower = col.lower()
    if ("decline" in col_lower or "unemployment" in col_lower):
        # Only process numeric columns
        if pd.api.types.is_numeric_dtype(bear_filtered[col]):
            col_mean = bear_filtered[col].abs().mean()
            # If mean abs value < 1, assume fraction and multiply by 100
            if np.isfinite(col_mean) and col_mean < 1:
                vals = bear_filtered[col] * 100
            else:
                vals = bear_filtered[col]
            # Format as string percentage with one decimal place
            bear_filtered[col] = vals.apply(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")
        else:
