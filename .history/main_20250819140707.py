...
st.markdown(f"**Your beginning investment value is: ${investment_amount:,.0f}**")
st.markdown(f"**The Ending (nominal) Portfolio Value Dividends Reinvested is: ***${ending_value:,.0f***}**")
    ...
    if not filtered_df.empty and all(col in filtered_df.columns for col in ["Nominal Dividends", "Composite"]):
        ...
        if pd.notnull(end_div_dollars):
            ...
            if pd.notnull(ending_value):
                ...
                st.markdown(f"**Using a withdrawal % of {custom_pct:.1f}% of your Ending (nominal) Portfolio Value, you could spend: ${custom_value:,.0f}**")
    else:
