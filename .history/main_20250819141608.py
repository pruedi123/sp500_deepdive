...
if average_decline is not None and average_duration is not None:
    bad_news_text = (
        ...
        f"* During the period you chose, there were {count} bear markets "
        f"* (temporary declines of 20% or more).\n\n"
        f"* The average of those temporary declines was {average_decline*100:.2f}%.\n\n "
        f"* The average duration of the bear market(s) was {int(round(average_duration))} days.\n\n"
        f"* We also experienced {recession_count} recessions during this time.\n\n"
        f"Now the good news..."
    )
    summary_placeholder.markdown(bad_news_text)

    # --- Bear/recession metrics cards section ---
    # Four cards: Number of Bear Markets, Average Bear Market Decline, Average Bear Market Duration, Number of Recessions
    card1_title = "Number of Bear Markets"
    card1_value = count
    card2_title = "Average Bear Market Decline"
    card2_value = f"{average_decline*100:.2f}%"
    card3_title = "Average Bear Market Duration"
    card3_value = f"{int(round(average_duration))} days"
    card4_title = "Number of Recessions"
    card4_value = recession_count

    # Layout: two rows of two
    row1_col1, row1_col2 = st.columns(2)
    with row1_col1:
        metric_card(card1_title, card1_value)
    with row1_col2:
        metric_card(card2_title, card2_value)
    row2_col1, row2_col2 = st.columns(2)
    with row2_col1:
        metric_card(card3_title, card3_value)
    with row2_col2:
        metric_card(card4_title, card4_value)
