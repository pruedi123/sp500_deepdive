...
# --- Compute averages for decline and duration columns BEFORE formatting ---
...
if decline_cols and pd.api.types.is_numeric_dtype(bear_filtered[decline_cols[0]]):
    average_decline = bear_filtered[decline_cols[0]].mean()
if duration_cols and pd.api.types.is_numeric_dtype(bear_filtered[duration_cols[0]]):
    average_duration = bear_filtered[duration_cols[0]].mean()


# --- Explicitly format Decline (%) and Peak Unemployment (%) columns as percentages with one decimal place ---
# Identify columns by exact name match (case-insensitive)
decline_col_name = None
unemployment_col_name = None
for col in bear_filtered.columns:
    if col.strip().lower() == "decline (%)":
        decline_col_name = col
    if col.strip().lower() == "peak unemployment (%)":
        unemployment_col_name = col

if decline_col_name is not None:
    # Convert to numeric, multiply by 100, format as string with one decimal and percent sign
    bear_filtered[decline_col_name] = pd.to_numeric(bear_filtered[decline_col_name], errors="coerce") * 100
    bear_filtered[decline_col_name] = bear_filtered[decline_col_name].apply(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")
if unemployment_col_name is not None:
    bear_filtered[unemployment_col_name] = pd.to_numeric(bear_filtered[unemployment_col_name], errors="coerce") * 100
    bear_filtered[unemployment_col_name] = bear_filtered[unemployment_col_name].apply(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")

st.markdown(f"**Number of bear markets in period: {count}**")
if show_bear_table == "Yes":
    st.dataframe(bear_filtered)

# Display averages for decline and duration columns (using pre-format values)
if decline_cols and average_decline is not None:
    formatted_decline = f"{average_decline*100:.2f}%"
    st.write("Average Bear Market Decline:", formatted_decline)
if duration_cols and average_duration is not None:
    formatted_duration = int(round(average_duration))
    st.markdown(f"Average Bear Market Duration (days): **{formatted_duration}**")
