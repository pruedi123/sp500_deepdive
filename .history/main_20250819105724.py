...
default_end = "2025-07"
begin = st.sidebar.selectbox("Start (YYYY-MM)", options=available_months, index=available_months.index(default_begin) if default_begin in available_months else 0)
end = st.sidebar.selectbox("End (YYYY-MM)", options=available_months, index=available_months.index(default_end) if default_end in available_months else len(available_months)-1)
