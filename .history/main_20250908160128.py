...
def metric_card(title, value):
    ...
    ">
        ...
        <div style="font-size:2.1rem;font-weight:700;color:#e53935;text-align:center;">
            {value}
        </div>
    </div>
    """
...
if decline_col is not None:
    ...
    # Keep raw numeric for averages; formatting for tables happens later if needed
...
if not bear_filtered.empty:
    ...
    for col in bear_filtered.columns:
        n = _norm(col)
        if any(tok in n for tok in ["end", "ended", "enddate", "enddate", "trough", "troughdate", "lowdate"]):
...
if average_decline is not None and average_duration is not None:
    ...
    if show_bear_table == "Yes":
        ...
        # Detect an End-Date column for display (common headers: End, Ended, End Date, Trough)
        end_col = None
        for col in bear_filtered.columns:
            n = _norm(col)
            if any(tok in n for tok in ["end", "ended", "enddate", "trough", "troughdate", "lowdate"]):
                end_col = col
                break

        # Ensure start and end are proper datetimes for display
        bear_filtered[bear_date_col] = pd.to_datetime(bear_filtered[bear_date_col], errors="coerce")
        if end_col is not None:
            if pd.api.types.is_numeric_dtype(bear_filtered[end_col]):
                bear_filtered[end_col] = pd.to_datetime(bear_filtered[end_col], origin="1899-12-30", unit="D", errors="coerce")
            else:
                bear_filtered[end_col] = pd.to_datetime(bear_filtered[end_col], errors="coerce")

        # Reorder columns so Start Date, End Date come first
        cols = list(bear_filtered.columns)
        ordered = []
        if bear_date_col in cols:
            ordered.append(bear_date_col)
        if end_col is not None and end_col in cols and end_col != bear_date_col:
            ordered.append(end_col)
        remainder = [c for c in cols if c not in ordered]
        display_df = bear_filtered[ordered + remainder].copy()

        # Rename for display clarity
        rename_map = {bear_date_col: "Start Date"}
        if end_col is not None:
            rename_map[end_col] = "End Date"
        display_df = display_df.rename(columns=rename_map)

        # Ensure unique column names (PyArrow/Streamlit disallow duplicates)
        def _dedup_cols(cols):
            ...
            for c in cols:
                if c in seen:
                    ...
                    out.append(c)
            return out
        display_df.columns = _dedup_cols(list(display_df.columns))
