...
# C CPI column calculations and display
if not filtered_df.empty and "C CPI" in filtered_df.columns:
    begin_c_cpi = filtered_df["C CPI"].iloc[0]
    end_c_cpi = filtered_df["C CPI"].iloc[-1]
    try:
        c_cpi_factor = end_c_cpi / begin_c_cpi if begin_c_cpi != 0 else float('nan')
    except Exception:
        c_cpi_factor = float('nan')
    st.write("Begin C CPI:", begin_c_cpi)
    st.write("End C CPI:", end_c_cpi)
    st.write("C CPI Factor:", f"{c_cpi_factor:.2f}x")
